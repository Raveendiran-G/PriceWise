...
function CompareTable({ stores = [] }) {
  if (!stores.length) {
    return (
      <div className="mt-10">
        <h2 className="text-3xl font-bold mb-6">Compare Prices</h2>
        <p className="text-gray-500">No store prices available.</p>
      </div>
    );
  }

  const lowestPrice = Math.min(...stores.map((item) => item.price));
